...
public class StorageVolumePreference extends Preference {
    ...
    public StorageVolumePreference(Context context, VolumeInfo volume, int color) {
        ...
        if (volume.isMountedReadable()) {
            ...
            android.util.Log.d("huang","totalBytes>>>"+totalBytes);
            if(path.getPath().toString().contains("/data")) {
                usedBytes += memorySize - totalBytes;
                totalBytes = memorySize;
            }

            final String used = Formatter.formatFileSize(context, usedBytes);
            final String total = Formatter.formatFileSize(context, Utils.getAlignedSize(totalBytes));
