        ...
        for (VolumeInfo vol : volumes) {
            if (vol.getType() == VolumeInfo.TYPE_PRIVATE) {
                ...
                if (vol.isMountedReadable()) {
                    ...
                    privateTotalBytes += Utils.getAlignedSize(path.getTotalSpace());
                    if(vol.getPath().toString().contains("/data")) {
                        systemBytes = memorySize - path.getTotalSpace();
                        privateUsedBytes += systemBytes;
                        privateTotalBytes = memorySize;
                    }
                }
        ...
        private void updateSummary() {
            ...
            for (VolumeInfo info : volumes) {
				...
				if (info.isMountedReadable()) {
					privateUsedBytes += path.getTotalSpace() - path.getFreeSpace();
					privateTotalBytes += Utils.getAlignedSize(path.getTotalSpace());
					if(info.getPath().toString().contains("/data")) {
						systemBytes = memorySize - path.getTotalSpace();
						privateUsedBytes += systemBytes;
						privateTotalBytes = memorySize;
					}
