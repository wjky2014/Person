...
public class PrivateVolumeSettings extends SettingsPreferenceFragment {
    ...
    private static final int[] ITEMS_SHOW_SHARED = new int[] {
			...
			R.string.header_category_system//zfl
    };
    ...
    private void update() {
        ...
        final long memorySize = Utils.getTotalRomSize();
        final File file = mVolume.getPath();
        final long totalBytes = file.getTotalSpace();
        final long freeBytes = file.getFreeSpace();
        final long usedBytes = totalBytes - freeBytes;
        boolean isInternal = mVolume.getPath().toString().contains("/data");
		android.util.Log.d(TAG,"mVolume.getPath()>>>"+mVolume.getPath());
		if(isInternal){
            systemBytes = memorySize - totalBytes;
 
		}
        final BytesResult result = Formatter.formatBytes(getResources(), isInternal? systemBytes + usedBytes :usedBytes, 0);
        mSummary.setTitle(TextUtils.expandTemplate(getText(R.string.storage_size_large),
                result.value, result.units));
        if(isInternal) {
            mSummary.setSummary(getString(R.string.storage_volume_used,
                    Formatter.formatFileSize(context, Utils.getAlignedSize(memorySize))));
            mSummary.setPercent((int) (((usedBytes + systemBytes) * 100) / memorySize));
        }else {
            mSummary.setSummary(getString(R.string.storage_volume_used,
                    Formatter.formatFileSize(context, Utils.getAlignedSize(totalBytes))));
    ...
    public boolean onPreferenceTreeClick(Preference pref) {
        ...
        switch (itemTitleId) {
			...
			//zfl
			case R.string.header_category_system: {
                return true;
        ...
        for (int i = 0; i < mItemPoolIndex; ++i) {
            ...
            switch (itemTitleId) {
				...
				case R.string.header_category_system: {
                    ...
                    final File file = mVolume.getPath();
                    final long totalBytes = file.getTotalSpace();
					long systemBytes = memorySize - totalBytes;
					//Log.e("qqq","!!!!!!memorySize="+memorySize+"  totalBytes="+totalBytes+" systemBytes="+systemBytes);
                    updatePreferenceSystem(item, systemBytes);
                } break;
				//zfl
                case R.string.storage_detail_cached: {
	...
	//zfl
	 private void updatePreferenceSystem(StorageItemPreference pref, long size) {
        pref.setStorageSizeSystem(size, Utils.getTotalRomSize());
    }
    //zfl
