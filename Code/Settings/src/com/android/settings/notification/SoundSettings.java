...
public class SoundSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    private Preference mPhoneRingtonePreference2;
    ...
    private void initRingtones() {
        ...
        mPhoneRingtonePreference2 = getPreferenceScreen().findPreference(KEY_PHONE_RINGTONE2);
        if (mPhoneRingtonePreference != null && !mVoiceCapable) {
            getPreferenceScreen().removePreference(mPhoneRingtonePreference);
            getPreferenceScreen().removePreference(mPhoneRingtonePreference2);
            mPhoneRingtonePreference = null;
            mPhoneRingtonePreference2 = null;
        }
        if (mPhoneRingtonePreference2 != null && !isMultiSim()) {
            mPhoneRingtonePreference.setTitle(R.string.ringtone_title);
            getPreferenceScreen().removePreference(mPhoneRingtonePreference2);
        }
        ...
        getPreferenceScreen().removePreference(mAlarmRingtonePreference);   //added by spro jzy
    }
    ...
    /** Spro: added by jzy for multisim @ { */
    private boolean isMultiSim() {
        return !"ss".equals(SystemProperties.get("persist.radio.multisim.config"));
    }
    /** @ } */
        ...
        public void run() {
            if (mPhoneRingtonePreference != null) {
                final CharSequence summary = updateRingtoneName(
                        mContext, RingtoneManager.TYPE_RINGTONE, 0);
                if (summary != null) {
                    mHandler.obtainMessage(H.UPDATE_PHONE_RINGTONE, summary).sendToTarget();
                }
            }
            if (mPhoneRingtonePreference2 != null) {
                final CharSequence summary = updateRingtoneName(
                        mContext, RingtoneManager.TYPE_RINGTONE, 1);
                if (summary != null) {
    ...
    /** Spro: added by jzy for multisim @ { */
    private static CharSequence updateRingtoneName(Context context, int type, int phoneId) {
        if (context == null) {
            Log.e(TAG, "Unable to update ringtone name, no context provided");
            return null;
        }
        Uri ringtoneUri = RingtoneManager.getActualDefaultRingtoneUri(context, type,phoneId);
        CharSequence summary = context.getString(com.android.internal.R.string.ringtone_unknown);
        // Is it a silent ringtone?
        if (ringtoneUri == null) {
            summary = context.getString(com.android.internal.R.string.ringtone_silent);
    ...
    private final class H extends Handler {
        ...
        private static final int UPDATE_PHONE_RINGTONE2 = 12;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case UPDATE_PHONE_RINGTONE2:
                    mPhoneRingtonePreference2.setSummary((CharSequence) msg.obj);
                    break;
