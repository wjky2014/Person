...
public class AccessibilitySettings extends SettingsPreferenceFragment implements DialogCreatable,
    ...
    private static final String TOGGLE_LED_FLASH_FOR_CALL_PREFERENCE =
            "toggle_led_flash_for_call_preference";

    //flip mute
    private static final String CALL_FLIP_MUTE =
            "call_flip_mute_preference";
    private static final String ALARM_FLIP_MUTE =
            "alarm_flip_mute_preference";
    ...
    private SwitchPreference mToggleLedForCallPreference;
    /// M: IPO preference
    private SwitchPreference mIpoSetting;
    private SwitchPreference mCallFlipMutePreference;
    private SwitchPreference mAlarmFlipMutePreference;
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (mSelectLongPressTimeoutPreference == preference) {
            ...
            return true;
        } else if(mCallFlipMutePreference == preference) {
            boolean doflip = (Boolean) newValue;
            Settings.Global.putInt(getContext().getContentResolver(),
                    "filp_to_mute", doflip ? 1 : 0);
            return true;
        } else if(mAlarmFlipMutePreference == preference) {
            boolean doflip = (Boolean) newValue;
            Settings.Global.putInt(getActivity().getContentResolver(),
                     "mute_alarm", doflip ? 1 : 0);
            return true;
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        if (mToggleHighTextContrastPreference == preference) {
            ...
            return true;
        }else if (mToggleLedForCallPreference == preference) {
            handleToggleLedForCallPreferenceClick();
            return true;
         }
        return super.onPreferenceTreeClick(preference);
    }
    private void handleToggleLedForCallPreferenceClick() {
        Settings.Secure.putInt(getContentResolver(),
                  "led_flash_for_call_enabled",
                  (mToggleLedForCallPreference.isChecked() ? 1 : 0));
   }
    ...
    private void initializeAllPreferences() {
        ...
        //Flip mute
        mCallFlipMutePreference = (SwitchPreference) findPreference(CALL_FLIP_MUTE);
        mCallFlipMutePreference.setOnPreferenceChangeListener(this);
        mAlarmFlipMutePreference = (SwitchPreference) findPreference(ALARM_FLIP_MUTE);
        mAlarmFlipMutePreference.setOnPreferenceChangeListener(this);
        ...
        // LED for call.
        mToggleLedForCallPreference =
                (SwitchPreference) findPreference(TOGGLE_LED_FLASH_FOR_CALL_PREFERENCE);
        ...
        if ("Xolo".equalsIgnoreCase(SystemProperties.get("ro.product.brand"))){
        ...
        mFontSizePreferenceScreen = (PreferenceScreen) findPreference(
                FONT_SIZE_PREFERENCE_SCREEN);
        //remove font size from accessibility
        if (true){
    ...
    private void updateSystemPreferences() {
        ...
        //for flip mute
        mCallFlipMutePreference.setChecked(Settings.Global.getInt(getContentResolver(),
                "filp_to_mute", 0) == 1);
        mAlarmFlipMutePreference.setChecked(Settings.Global.getInt(getContentResolver(),
                "mute_alarm", 0) == 1);
        ...
        if (KeyCharacterMap.deviceHasKey(KeyEvent.KEYCODE_POWER)
         ...
         mToggleLedForCallPreference.setChecked(
               Settings.Secure.getInt(getContentResolver(),
                       "led_flash_for_call_enabled", 0) == 1);
