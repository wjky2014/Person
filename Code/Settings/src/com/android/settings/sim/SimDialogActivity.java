...
public class SimDialogActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        switch (dialogType) {
            ...
            case DEFAULT_DATA_PICK:
                int mSubId = extras.getInt("subId");
                if(SubscriptionManager.from(this).getDefaultDataSubscriptionId() == mSubId){
                    finish();
                } else {
                    changeDefaultData(mSubId);
                }
                break;
            default:
    ...
    private void changeDefaultData(int subId){
         setDefaultDataSubId(getApplicationContext(), subId);
         finish();
    }
    ...
    private void setDefaultDataSubId(final Context context, final int subId) {
        ...
        if (!TelephonyUtils.isRadioOn(subId, context)){
             Toast.makeText(context, R.string.deactivated_sim, Toast.LENGTH_SHORT).show();
             return;
        }
        ...
        Log.d(TAG, "data enableBefore: " + enableBefore + ", preSubId: " + preSubId);
                            ...
                            case DATA_PICK:
                                ...
                                if (!mSimManagementExt.switchDefaultDataSub(context, targetSub)) {
                                    ...
                                    /// M: for [C2K OMH Warning] @{
                                       ...
                                       // String spn = tm.getSimOperatorNameForPhone(1 - value);
                                        Phone mPhone = PhoneFactory.getPhone(1 - value);
                                        //Log.d(TAG, "DATA_PICK mPhone.isImsRegistered() = " + mPhone.isImsRegistered());
                                        if (/*"Jio".equalsIgnoreCase(spn)*/mPhone != null && mPhone.isImsRegistered()) {
                                            showDataAlertDialog(targetSub, value);
                                            return;
                                        } else {
    ...
    private void showDataAlertDialog(final int subId, final int phoneId) {
        dismissSimDialog();
        String attention = getResources().getString(R.string.switch_data_card_attention, phoneId + 1);
        AlertDialog alertDialog = new AlertDialog.Builder(this)
            .setTitle(attention)
            .setMessage(R.string.whether_switch_data_card)
            .setPositiveButton(android.R.string.ok,
                new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int which) {
                    setDefaultDataSubId(SimDialogActivity.this, subId);
                    finish();
                }
            })
