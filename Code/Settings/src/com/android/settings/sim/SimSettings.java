...
public class SimSettings extends RestrictedSettingsFragment implements Indexable {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            // listen to radio state change
                ...
                if (mRadioController.isRadioSwitchComplete(subId)) {
                    ...
                    changeData(subId , TelephonyUtils.isRadioOn(subId, mContext));
                }
            }
        }
    };
   ...
   private void changeData(int subId, boolean isRadioOn){
        int[] mSubIds = mSubscriptionManager.getActiveSubscriptionIdList();
        int dataSubId = subId;
        boolean needUpdateData = false;
        for (int i = 0; i < mSubIds.length; i++) {
            if(TelephonyUtils.isRadioOn(mSubIds[i], mContext) && !isRadioOn){
                 dataSubId = mSubIds[i];
                 needUpdateData = true;
            } else if (!TelephonyUtils.isRadioOn(mSubIds[i], mContext) && isRadioOn){
                 dataSubId = subId;
                 needUpdateData = true;
            }
        }
        Log.d(TAG, "needUpdateData = " + needUpdateData + "  dataSubId = " + dataSubId);
        if(!needUpdateData)  return;
        Intent newIntent = new Intent(mContext, SimDialogActivity.class);
        newIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        newIntent.putExtra(SimDialogActivity.DIALOG_TYPE_KEY, SimDialogActivity.DEFAULT_DATA_PICK);
        newIntent.putExtra("subId", dataSubId);
