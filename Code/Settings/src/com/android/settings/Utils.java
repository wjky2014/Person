...
public final class Utils extends com.android.settingslib.Utils {
    ...
    public static boolean updatePreferenceToSpecificActivityOrRemove(Context context,
        ...
        if (intent != null) {
            ...
            boolean isLegalInformation = false;
            for (int i = 0; i < listSize; i++) {
                ...
                if ((resolveInfo.activityInfo.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM)
                    ...
                    if (resolveInfo.activityInfo.packageName.equals("com.hive.otamanager")) {
                        isLegalInformation = true;
                    }
                    if ((flags & UPDATE_PREFERENCE_FLAG_SET_TITLE_TO_MATCHING_ACTIVITY) != 0) {
                        // Set the preference title to the activity's label
                        if(android.os.Build.CUSTOMER.equalsIgnoreCase("Lava") && isLegalInformation){
                            preference.setTitle("Software updates");
                         }else{
...
//zfl
    ...
    public static long getAlignedSize(long size) {
        long s = 1;
        do {
            s = s << 1;
        }while(s < size);
        Log.d(TAG, "getAlignedSize "+ size + "->" + s);
        return s;
    }
	//zfl
}
